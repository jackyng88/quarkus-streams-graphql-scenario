...
### Javascript Section
- We have 5 imports here. The first being the Socket from `socket.js` so we can properly use the WebSocket. The next 4
...
to be used.
- Within the `apollo` we have an `allScores` object whose `query:` field is being called with `GET_ALL_SCORES`.
- Next in `methods:` we have 4 functions - `handleMessage()`, `checkScores()`, `updatesScres()`, and `createScores()`. The first two
...
to create 4 different Javascript files.
